import pygame
from sys import exit
from random import randint, choice


class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        player_frame_1 = pygame.image.load('graphics/player/player_walk_1.png').convert_alpha()
        player_frame_2 = pygame.image.load('graphics/player/player_walk_2.png').convert_alpha()
        self.player_walk = [player_frame_1, player_frame_2]
        self.player_index = 0
        self.player_jump = pygame.image.load('graphics/player/jump.png').convert_alpha()

        self.image = self.player_walk[self.player_index]
        self.rect = self.image.get_rect(midbottom=(80, 300))
        self.gravity = 0

        self.jump_sound = pygame.mixer.Sound('audio/jump.mp3')
        self.jump_sound.set_volume(.1)

    def player_input(self):
        keys = pygame.key.get_pressed()
        if keys[pygame.K_SPACE] and self.rect.bottom >= 300:
            self.jump_sound.play()
            self.gravity = -20
        if keys[pygame.K_UP] and self.rect.bottom >= 300:
            self.jump_sound.play()
            self.gravity = -20

    def apply_gravity(self):
        self.gravity += 1
        self.rect.y += self.gravity
        if self.rect.bottom >= 300:
            self.rect.bottom = 300

    def animation_state(self):
        if self.rect.bottom < 300:
            self.image = self.player_jump
        else:
            self.player_index += 0.1
            if self.player_index >= len(self.player_walk):
                self.player_index = 0
            self.image = self.player_walk[int(self.player_index)]

    def update(self):
        self.player_input()
        self.apply_gravity()
        self.animation_state()


class Obstacle(pygame.sprite.Sprite):
    def __init__(self, type):
        super().__init__()
        if type == 'snail':
            snail_frame_1 = pygame.image.load('graphics/snail/snail_1.png').convert_alpha()
            snail_frame_2 = pygame.image.load('graphics/snail/snail_2.png').convert_alpha()
            self.frames = [snail_frame_1, snail_frame_2]
            y_pos = 300
        else:
            fly_frame_1 = pygame.image.load('graphics/fly/fly_1.png').convert_alpha()
            fly_frame_2 = pygame.image.load('graphics/fly/fly_2.png').convert_alpha()
            self.frames = [fly_frame_1, fly_frame_2]
            y_pos = 210

        self.animation_index = 0
        self.image = self.frames[self.animation_index]
        self.rect = self.image.get_rect(midbottom=(randint(900, 1100), y_pos))

    def animation_state(self):
        self.animation_index += 0.1
        if self.animation_index >= len(self.frames):
            self.animation_index = 0
        self.image = self.frames[int(self.animation_index)]

    def destroy(self):
        if self.rect.x <= -100:
            self.kill()

    def update(self):
        self.animation_state()
        self.rect.x -= 6
        self.destroy()


def collision():
    if pygame.sprite.spritecollide(player.sprite, enemies, False):
        enemies.empty()
        return False
    else:
        return True


# game score
def display_score():
    current_time = int(pygame.time.get_ticks() / 1000) - start_time
    score_surf = text_font.render(f'Score: {current_time}', False, (64, 64, 64))
    score_rec = score_surf.get_rect(center=(400, 50))
    screen.blit(score_surf, score_rec)

    return current_time


pygame.init()

# important information pygame
screen = pygame.display.set_mode((800, 400))
pygame.display.set_caption('Infinity Jumps')
game_active = False

# text options
text_font = pygame.font.Font('font/Pixeltype.ttf', 50)

# game clock
clock = pygame.time.Clock()
start_time = 0
score = 0
background_sound = pygame.mixer.Sound('audio/music.wav')
background_sound.set_volume(.1)
background_sound.play(loops=-1)

# groups
player = pygame.sprite.GroupSingle()
player.add(Player())

enemies = pygame.sprite.Group()

# background
sky_surface = pygame.image.load('graphics/sky.png').convert()
ground_surface = pygame.image.load('graphics/ground.png').convert()

# game intro
player_stand = pygame.image.load('graphics/player/player_stand.png').convert_alpha()
player_stand = pygame.transform.rotozoom(player_stand, 0, 2.5)
player_stand_rec = player_stand.get_rect(center=(400, 200))

game_name = text_font.render('Infinity Jump', False, (111, 196, 169))
game_name_rec = game_name.get_rect(center=(400, 50))

instructions = text_font.render('Press SPACE or UP to start', False, (111, 196, 169))
instructions_rec = instructions.get_rect(center=(400, 350))

# enemies timer
enemy_timer = pygame.USEREVENT + 1
pygame.time.set_timer(enemy_timer, 1500)

while True:
    for event in pygame.event.get():

        if event.type == pygame.QUIT:
            pygame.quit()
            exit()
        if game_active:
            if event.type == enemy_timer:
                enemies.add(Obstacle(choice(['fly', 'snail', 'snail', 'snail'])))
        else:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    game_active = True
                    start_time = int(pygame.time.get_ticks() / 1000)
                if event.key == pygame.K_SPACE:
                    game_active = True
                    start_time = int(pygame.time.get_ticks() / 1000)

    if game_active:

        game_active = collision()
        screen.blit(sky_surface, (0, 0))
        screen.blit(ground_surface, (0, 300))
        score = display_score()

        player.draw(screen)
        player.update()

        enemies.draw(screen)
        enemies.update()

    else:
        screen.fill((94, 129, 162))

        screen.blit(game_name, game_name_rec)

        screen.blit(player_stand, player_stand_rec)

        score_msg = text_font.render(f'Your score: {score}', False, (111, 196, 169))
        score_msg_rect = score_msg.get_rect(center=(400, 350))

        if score != 0:
            screen.blit(score_msg, score_msg_rect)
        else:
            screen.blit(instructions, instructions_rec)

    pygame.display.update()
    clock.tick(60)
